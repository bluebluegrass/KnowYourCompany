...
const SECTION_QUERIES: Record<SectionId, (input: InputContext) => string[]> = {
  ...
  financial_health: ({ company }) => [
    `"${company}" latest earnings revenue net income quarterly results`,
    `"${company}" investor relations annual report OR quarterly report OR financial results`,
    `"${company}" funding round valuation investors`,
    `"${company}" IPO OR "going public" OR "initial public offering"`,
    `"${company}" acquisition OR "private equity" OR parent company ownership`,
    `"${company}" revenue OR profitability OR cash OR debt`
  ],
