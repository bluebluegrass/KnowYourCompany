...
async function readResearchInput(args: CliArgs, rl: readline.Interface): Promise<{ company: string; location: string; role: string; candidateProfile?: CandidateProfile }> {
  ...
  const candidateProfile = candidateProfileFromArgs(args);
  return {
...
async function runResearchMode(args: CliArgs, inputContext: { company: string; location: string; role: string; candidateProfile?: CandidateProfile }): Promise<void> {
  const reportInput = {
    company: inputContext.company,
    ...(inputContext.location ? { location: inputContext.location } : {}),
    ...(inputContext.role ? { role: inputContext.role } : {}),
    ...(inputContext.candidateProfile ? { candidateProfile: inputContext.candidateProfile } : {}),
...
function candidateProfileFromArgs(args: CliArgs): CandidateProfile | undefined {
  const minimum = args["minimum-compensation"];
  if (minimum && (!Number.isFinite(Number(minimum)) || Number(minimum) <= 0)) throw new Error("--minimum-compensation must be a positive number.");
  const priorities = args.priority?.split(",").map((value) => value.trim()).filter(Boolean) as CandidateProfile["priorities"];
  const dealBreakers = args["deal-breaker"]?.split(",").map((value) => value.trim()).filter(Boolean) as CandidateProfile["dealBreakers"];
  const profile: CandidateProfile = {};
  if (args.seniority) profile.seniority = args.seniority as NonNullable<CandidateProfile["seniority"]>;
  if (args["work-authorization"]) profile.workAuthorization = args["work-authorization"] as NonNullable<CandidateProfile["workAuthorization"]>;
  if (args["work-mode"]) profile.workModePreference = args["work-mode"] as NonNullable<CandidateProfile["workModePreference"]>;
  if (priorities?.length) profile.priorities = priorities;
  if (dealBreakers?.length) profile.dealBreakers = dealBreakers;
  if (minimum) profile.minimumCompensation = { amount: Number(minimum), currency: args.currency || "EUR", period: "year" };
