...
export async function renderFromJson(jsonPath: string, outputDir?: string): Promise<{ report: ReportModel | ReportV2; html: string; outputPath: string }> {
...
function parseReport(raw: string, jsonPath: string): ReportModel | ReportV2 {
  let value: unknown;
  try {
  ...
  if (report.reportVersion === 2) {
    ...
    return report as unknown as ReportV2;
  }
...
function isReportV2(report: ReportModel | ReportV2): report is ReportV2 { return "reportVersion" in report && report.reportVersion === 2; }
